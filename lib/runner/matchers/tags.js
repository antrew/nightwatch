...
class TagsMatcher {
  ...
  containsAllTags(moduleTags) {
    return this.tagsToIncludeArray.every(tagToInclude => moduleTags.includes(tagToInclude));
  }
  ...
  checkModuleTags(testModule) {
    ...
    if (this.hasIncludeTagFilter() && !this.containsAllTags(moduleTags)) {
      return false;
    }
